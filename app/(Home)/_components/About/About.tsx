import React from "react";

type Props = {};

const About = (props: Props) => {
  return (
    <div className="lg:px-10  py-20 lg:py-32 text-[25px] max-lg:font-semibold md:text-5xl  uppercase flex lg:justify-end font-medium tracking-tight ContainerPadding">
      <div className="lg:w-[65%] ">
        At CCCMII, we are dedicated to empowering individuals and organizations
        with the skills and knowledge necessary for effective mediation,
        negotiation, counselling, and conciliation
      </div>
    </div>
  );
};

export default About;
